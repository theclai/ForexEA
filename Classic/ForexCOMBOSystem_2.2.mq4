...
extern   int      xStopLoss = 110;
extern   int      xTakeProfit = 21;
int      MA1prd = 60;
...
extern   int      xTakeProfit_II = 300;
extern   int      xStopLoss_II = 30;
...
extern   double   xTakeProfit_III = 160;
extern   int      xStopLoss_III = 70;
...
double   point,TakeProfit,TakeProfit_II,TakeProfit_III,StopLoss,StopLoss_II,StopLoss_III;
...
StopLevel = MarketInfo(Symbol(),MODE_STOPLEVEL);
//if ((Year() > LastYear) || ((Month() > LastMonth) && (Year() == LastYear)) || ((Day() > LastDay) && (Month() >= LastMonth) && (Year() == LastYear)))
//   {
//   Comment(" This version is out of date. Visit www.forex-combo.com and get your updated version!");
//   Alert("This version is out of date. Visit www.forex-combo.com and get your updated version!");
//   Sleep(30000);
//   return(0);
//   }
...
//handle = 1;
//wInit(AccountNumber(),IsTesting(),IsDemo());
...
if (xTakeProfit < StopLevel) TakeProfit = StopLevel;else TakeProfit=xTakeProfit;
if (xTakeProfit_II < StopLevel) TakeProfit_II = StopLevel;else TakeProfit_II=xTakeProfit_II;
if (StopLoss < StopLevel) StopLoss = StopLevel;else StopLoss=xStopLoss;
if (StopLoss_II < StopLevel) StopLoss_II = StopLevel;else StopLoss_II=xStopLoss_II;
if (TakeProfit_III < StopLevel) TakeProfit_III = StopLevel;else TakeProfit_III=xTakeProfit_III;
if (StopLoss_III < StopLevel) StopLoss_III = StopLevel;else StopLoss_III=xStopLoss_III;
